% This function generates the figures that explore the effect changing the population size
% Figures are reported in section 2 of the supplementary material.


clear all
close all
clc
load ContentOfGOssip_populationSize.mat
...
for j=1:5       
        ...
        ax = gca; % current axes
        ax.FontSize = 14;
        title({'Average cooperation level for'; 'different contents of gossip'; ['Population size: ' num2str(round(NN(j)))]},'FontSize',20);
        ylabel('Proportion of Cooperation','FontSize',20);
        xlabel('Time','FontSize',20);
        legend('no gossip','Pass R_{zi} and c_z','Pass only C_z','location','northwest');
        name_file=['Temporal_Fig1_populationsize_' num2str(round(NN(j)))];
        ...
        sgtitle({'Distribution of cooperation levels at t_{max}'; ['Population size: ' num2str(round(NN(j)))]}, 'FontSize',20)
        name_file=['Outcome_Fig2_populationsize_' num2str(round(NN(j)))];
        saveas(numfig,name_file,'png')
...
clear all
close all
load OutDatedInfoVsAmountGossip_populationsize.mat
...
% 
 numfig=1;
 for i=1:length(NN_vct) % count deltas    
         index_to_print=find(NN==NN_vct(i)); % select current Delta
         ...
         legend('Pass R_{zi} and c_z - Info ages',...
             ...
             'Location','southeast');
         title({'Average cooperation level for different'; 'contents of gossip and information characteristics'; ['Population size: ' num2str(round(NN_vct(i)))]},'FontSize',20);
         ylabel('Proportion of Cooperation','FontSize',20);
         xlabel('Relative amount of gossip','FontSize',20);
         saveas(numfig,['RelativeAmountGossip_Fig4_populationsize_' num2str(round(NN_vct(i)))],'png');
         numfig=numfig+1;
 end
