...
%(section 1.4 of supplementary material).
% It also generates the figure 2 of the main text.
% If a cluster it is not available it is sufficient to substitute
% istructions from line 23 to line 89 with the call to the function:
% OutDatedInfoVsAmountGossip_alternative(init,fin,iter);
% The function generates one .mat file for each 100 simulations, which can
% be then aggregated in a single mat file with the aggregate03.m script.
...
for init=1:maximumval
    ...
    for iter=1:100
        ...
        task = createTask (myJob1, @OutDatedInfoVsAmountGossip_alternative, 0, {init,fin,iter});
        submit (myJob1);
