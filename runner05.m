...
% (section 3.3 of supplementary material).
% If a cluster it is not available it is sufficient to substitute
% istructions from line 21 to line 85 with the call to the function:
% OutDatedInfoVsAmountGossip_theta(init,fin);
% The function generates one .mat file for each 100 simulations, which can
% be then aggregated in a single mat file with the aggregate05.m script.
...
while init<=maximumval
        ...
        task = createTask (myJob1, @OutDatedInfoVsAmountGossip_theta, 0, {init,fin});
        submit (myJob1);
