...
% the effect of theta (the size of change in reputation) on the evolution of the model.
% In particular it produces the data for the temporal analyses (section 3.1
% of Supplementary material), and the relative distributions (sections 3.2).
% If a cluster it is not available it is sufficient to substitute
% istructions from line 19 to line 84 with the call to the function:
% ContentOfGossip_temporal_theta(init,final);
% The function generates one .mat file for each 100 simulations, which can
% be then aggregated in a single mat file with the aggregate06.m script.
...
while final<=maximumvalue
    ...
    task = createTask (myJob1, @ContentOfGossip_temporal_theta, 0, {init,final});
    submit (myJob1);
    ...
    if init>maximumvalue; break; end
end
